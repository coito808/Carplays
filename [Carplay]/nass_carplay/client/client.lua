...
AddEventHandler("nass_carplay:playsound", function(data)
    ...
    elseif data.event == "clearSongsBtn" then
        if not exports.xsound:soundExists("nass_carplay_"..data.vehStr) then return end
    ...
    while true do
        ...
        if veh ~= 0 then
            ...
            if not IsVehicleEngineOn(veh) then
                -- If the engine is off, destroy the sound
                if exports.xsound:soundExists("nass_carplay_"..data.vehStr) then
                    exports.xsound:Destroy("nass_carplay_"..data.vehStr)
                    print("Successfully destroyed sound for ID: nass_carplay because vehicle engine is off")
                else
                    print("Error: Sound does not exist for ID: nass_carplay")
                end
            end
        end
    end
