...
clearSongsBtn.addEventListener('click', function() {
const songList = document.querySelector('.song-list');
while (songList.firstChild) {
  deleteSong(songList.firstChild); // Call deleteSong on each song item

  function deleteSong(songListItem) {
    var songList = document.querySelector(".nextcontainer .song-list");
    songList.removeChild(songListItem);
  
    // Notify the server to stop the song
    $.post(`https://${GetParentResourceName()}/callback`, JSON.stringify({
      event: 'deleteSong',
      vehStr: vehIdStr,
      veh: vehId,
